...
# Data are loaded from file "migrations_preprocessed.xlsx" in which already 
# contain most relevant data, such as:
#    - GMID, X, Y coordinates of Origin
#    - GMID, X, Y coordinates of Destination
#    - apiary type at origin, apiary type at destination
#    - air distances between origin and destination
# 
# 1. We import data with all the short distance migration included
# 2. We import data with < 3 km migrations removed and fuel prices included.
...
f0.savefig(f'{CONFIG["figure_dir"]}/colony_moves_by_year.pdf')
...
f000 = plt.figure(figsize=(8, 4), dpi=200); a00_000 = f000.add_axes([0.15, 0.15, 0.70, 0.75]); a00_000_twin = a00_000.twinx()

sns.set_style('white')
sns.barplot(KMG_MID_migrated, x="year", y="n_of_migrations", ax=a00_000, color = "darkgrey")
sns.barplot(KMG_MID_migrated_augmented, x="year", y="n_of_migrations", ax=a00_000_twin, color = "teal")
df_movements_yearly = df_migrations.groupby(df_migrations.year)["uuid"].count().reset_index(name="n_of_migrations")
df_movements_augmented_yearly = df_migrations_augmented.groupby(df_migrations_augmented.year)["uuid"].count().reset_index(name="n_of_migrations")


a00_000.set_ylabel("Number of migratory \n beekeepers per year", fontsize=15, color="grey")
a00_000.set_xlabel("Year", fontsize=15)
a00_000.tick_params(axis='y', labelcolor="darkgrey", labelsize=15)
a00_000_twin.set_ylim(a00_000.get_ylim())
a00_000_twin.set_ylabel("Number of migratory beekeepers\n > 8 colonies and > 5 km direct dist", color="teal", fontsize=15)
a00_000_twin.tick_params(axis='y', labelcolor="teal")
a00_000.tick_params(axis='both', which='major', labelsize=15)
a00_000_twin.tick_params(axis='both', which='major', labelsize=15)


sns.set_style('white')
f000.savefig(f'{CONFIG["figure_dir"]}/number_of_migrations_per_year.pdf')
...
grid2.fig.savefig(f'{CONFIG["figure_dir"]}/yearly_dynamics_of_migrations_at_week_scale.pdf')
...
f2_1.savefig(f'{CONFIG["figure_dir"]}colony_packaging_by_migration_augmented.pdf')
...
f2_1.savefig(f'{CONFIG["figure_dir"]}/packaging_of_colonies_in_move.pdf')
...
f6.savefig(f'{CONFIG["figure_dir"]}/yearly_flow-related_cumulative_travel_distances-10-04.pdf')
...
# Number of apiaries by the same owner
# Load data
origins = CONFIG['origins']
df_origin = pd.read_excel(origins, sheet_name='Export Worksheet')
df_origin = df_origin[["KMG_MID", "GMID", "X_COORDINATE", "Y_COORDINATE", "CENSUS_2021_10"]]
df_origin.dropna(inplace=True)
df_digested = df_origin.groupby("KMG_MID")["GMID"].count().reset_index(name="št. čebelnjakov")

# Calculate distances between apiaries belonging to the same beekeeper
# 1. First, keep those KMG-MIDs, which have more than one apiary
# 2. Check total number of apiaries
# 3. Calculate direct distances  between those apiaries (Pythagoras)
# 4. dump to csv
df_digested = df_digested.loc[df_digested["št. čebelnjakov"] > 1]
df_subset = df_origin.loc[df_origin.KMG_MID.isin(df_digested.KMG_MID)]
KMGMID_list = df_digested.loc[df_digested["št. čebelnjakov"] > 1]["KMG_MID"].tolist()

df_dist = pd.DataFrame()
count = 0
for k in KMGMID_list:
    df_temp = df_subset.loc[df_subset.KMG_MID == k]
    gmid_list = list(it.combinations(df_temp.GMID.tolist(), 2))
    for g in gmid_list:
        df_dist.loc[count,'KMG_MID'] = k
        
        # Do Pythagoras
        kateta1 = (df_temp.loc[df_temp.GMID == g[0]].X_COORDINATE.values - df_temp.loc[df_temp.GMID == g[1]].X_COORDINATE.values)**2
        kateta2 = (df_temp.loc[df_temp.GMID == g[0]].Y_COORDINATE.values - df_temp.loc[df_temp.GMID == g[1]].Y_COORDINATE.values)**2
        hipotenuza = np.round(np.sqrt(np.abs(kateta2 + kateta1))/1000, 2)
        df_dist.loc[count, 'GMID1'] = g[0]
        df_dist.loc[count,'GMID2'] = g[1]
        
        # Convert to meters.
        df_dist.loc[count,'air_distance'] = hipotenuza
        count = count + 1

df_dist.to_csv(CONFIG['distances'], sep='\t')

# Draw distribution of distances
f_si_dist_KMGMID = plt.figure(figsize=(8, 4), dpi=200); a_SI_dist_KMGMID = f_si_dist_KMGMID.add_axes([0.15, 0.15, 0.8, 0.75])
sns.histplot(data=df_dist, x="air_distance", ax=a_SI_dist_KMGMID, binwidth=1)
a_SI_dist_KMGMID.set_xlabel("Zračna razdalje med čebelnjaki istega lastnika | Zračna razdalja med čebelnjaki istega lastnika ")
a_SI_dist_KMGMID.set_ylabel("Število | Number")

# Save stats for export later.
all_stats_interapiary_distances_same_owner = df_dist.air_distance.describe()
...
all_stats_interapiary_distances_same_owner.to_excel(writer, sheet_name='Interapiary distance')
